   ...
   <b-card
    ...
    <b-card-body class="">
     <b-card-header class="d-flex justify-content-between mb-3 px-0">
      <h3 class="d-flex align-self-center font-weight-bold"> {{itemid == null ? $t('forms.create-block') : $t('forms.edit-block')}}</h3>
     ...
     <b-form
      ...
      <b-form-group
       ...
       <b-form-input
        ...
        :dir="direction" />
       <b-form-invalid-feedback v-if="!$v.blockForm.name_ar.required">{{`${$t('forms.name-ar')}
      ...
      <b-form-group
       ...
       <b-form-input
        ...
        :dir="direction" />
       <b-form-invalid-feedback v-if="!$v.blockForm.name_en.required">{{`${$t('forms.name-en')}
              ${$t('validations.required')}`}}!
       </b-form-invalid-feedback>
      </b-form-group>
      <b-form-group
      ...
      <b-form-group
       ...
       <datepicker
        ...
        format="yyyy-MM-dd" />
       <b-form-invalid-feedback
        :class="{'d-block invalid-feedback':blockForm.post_date == ''}"
      ...
      <b-form-group>
       <div class="d-flex justify-content-start">
        <Switches
         ...
         color="primary mx-2 d-inline"
         class="vue-switcher-small d-flex align-self-center"></Switches>
        <h6 class="d-flex align-self-center">{{$t('forms.visible-in-website')}}</h6>
       </div>
      </b-form-group>
...
export default {
   ...
   if (this.itemid != null) {
    axios
     ...
     .then(res => {
      ...
      this.blockForm.description_ar = this.dataresult.locales.ar.description;
      this.blockForm.description_en = this.dataresult.locales.en.description;
      this.$v.blockForm.post_date.$model = new Date(this.dataresult.post_date);
  ...
  setData() {
   ...
   if (!this.$v.blockForm.$anyError) {
    ...
    if (this.blockForm.post_date != "") {
     this.formData.append("post_date", this.blockForm.post_date.toISOString());
    ...
    if (this.itemid != null) {
     ...
     axios //PUT method
      ...
      .then(
       res => {
        ...
        if (this.dataresult.data.success == true) {
         ...
         this.$notify("success", "Block has been Updated Successfully", null, { duration: 5000, permanent: false });
        } else {
         this.$notify("warning", "Block was not Updated", "please try again Later", null, { duration: 5000, permanent: false });
         this.disableSubmit = false
         //  window.location.href = '../../';
        }
       })
      .catch(error => {
       this.disableSubmit = false
       this.$notify("error", "Block was not Updated", "Network Error", null, { duration: 5000, permanent: false });
     ...
     axios //POST method
      ...
      .then(
       res => {
        ...
        if (this.dataresult.data.success == true) {
         this.$router.push('list')
         this.$notify("success", "Block has been Added Successfully", null, { duration: 5000, permanent: false });
         // window.location.href = '../../';
        } else {
         this.$notify("warning", "Block was not Added", "please try again Later", null, { duration: 5000, permanent: false });
         this.disableSubmit = false
         //  window.location.href = '../../';
        }
       })
      .catch(function (error) {
       this.disableSubmit = false
       this.$notify("error", "Block was not Added", "Network Error", null, { duration: 5000, permanent: false });
       console.log(error)
      });
    }
   }
 ...
 async mounted() {
  await this.getData()
  await axios.get(this.apiBase + '/categories')
   .then(res => {
