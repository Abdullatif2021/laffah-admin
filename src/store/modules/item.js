...
const getters = {
    ...
    selectData: (state) => {
        return Object.values(state.itemsList)
            .map(o => o.locales[this.$i18n.locale])
    },
...
const actions = {
    ...
    deleteBatch({ commit, dispatch, state }, payload) {
        ...
        return axios
            ...
            .then(response => {
                ...
                this._vm.$notify("success", "Deleted Successfully", null, { duration: 5000, permanent: false });
                return response
            })
            .catch(error => {
                this.$notify("danger", "Deleted Failed", "Try again later", { duration: 5000, permanent: false });
                console.log(error);
                return error
            })
    ...
    getItem({ commit, dispatch, rootGetters }, payload) {
        console.log(apiUrl + modelUrl + '/' + payload, 'apiUrl + modelUrl + \'/\' + payload')
        return axios
                ...
                if (parseInt(item.type) === 0) {
                    if (item.itemBatchBranches.length > 0) {
                        dispatch('fetchCities', null, { root: true }).then((res) => {
                            ...
                            item.prices = item.itemBatchBranches.map(o => ({...o, ...cities.filter(c => c.id == o.city_id)[0] }));
