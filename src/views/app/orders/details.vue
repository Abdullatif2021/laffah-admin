  ...
  <div>
    <b-row v-if="order">
      ...
      <b-colxx
        ...
        <b-button
          @click="print(order)"
          variant="outline-primary"><i class="simple-icon-printer"></i></b-button>
      </b-colxx>
      <b-colxx
        xxs="12"
        md="7"
        ...
        <div>{{ $t('forms.branch') }} :
          <b-badge
            ...
            variant="outline-primary">{{ order.branch_name }}
          </b-badge>
        </div>
        <div>
          {{ $t('pages.due') }}: <kbd>{{ formatDateTime(order.delivery_time, true) }}
          <span v-if="order.delivery_period">({{ deliveryPeriod(order.delivery_period) }})</span></kbd>
        </div>
      </b-colxx>
      <b-colxx
        xxs="12"
          ...
          <b-card
            ...
            <template #header>
              <div class="d-flex justify-content-between">
                <h6 class="mb-0 text-muted"><span class="iconsminds-financial"/> {{ $t('pages.payment-details') }}</h6>
                <b-button
                  ...
                  <small>{{ ((statuses || {})[initState] || '_').replace('_', ' ') }}&nbsp;</small>
                  <b-icon
                    icon="gear-fill"
                    font-scale="1"
                    aria-hidden="true"
            ...
            <template #footer>
              <h5 class="d-flex justify-content-between">Total: <b>{{ $t('pages.aed') }} {{ order.total }}</b></h5>
            </template>
            <b-card-text
...
<!--            <b-card-text class="mb-1 text-nowrap ">
            ...
            <b-card-text
              v-if="order.qty !== null"
              class="list-item-heading mb-1 truncate d-flex justify-content-between">{{ $t("pages.qty") }}:
              <kbd>#{{ order.qty }}</kbd>
            </b-card-text>
            <br>
            <b-card-text
              v-if="order.subtotal !== null"
              class="list-item-heading mb-1 truncate d-flex justify-content-between">{{ $t("pages.subTotal") }}: <b>
              {{ order.total_before_vat }}</b>
            </b-card-text>
            <b-card-text
              v-if="order.discount_value !== null"
              class="list-item-heading mb-1 truncate d-flex justify-content-between">{{ $t("pages.discount_value") }}:
              <b> {{ order.discount_value }}</b>
            </b-card-text>
            <b-card-text
              v-if="order.vat_value !== null"
              class="list-item-heading mb-1 truncate d-flex justify-content-between">{{ $t("pages.vat") }}: <b>
              {{ order.vat_value }}</b>
            </b-card-text>
            <br>
            <b-card-text
          ...
          <b-card
            ...
            <template #header>
              <h6 class="mb-0 text-muted"><span class="simple-icon-user"/> {{ $t('pages.contact-info') }}</h6>
            </template>
          ...
          <b-card
            ...
            header-tag="header">
            <template #header>
              <h6 class="mb-0 text-muted"><strong class="iconsminds-scooter"/> {{ $t('pages.delivery-address') }}</h6>
            </template>
            <b-card-title>
        ...
        {{ order.address.region }},
              ...
              <span>{{ order.address.street }},
       </span>
              <span>{{ order.address.building }},
       </span>
              <span>{{ order.address.floor }}
       </span>
            </b-card-title>
            <b-button
          ...
          <b-card
            ...
            <b-card-text
              ...
              Used: <b>{{ order.promotion.used }}-{{ order.promotion.max_use }}</b>
            </b-card-text>
            <b-card-text
              v-if="order.qty !== null"
              class="list-item-heading mb-1 truncate small text-center">{{ order.promotion.from_date }}
            </b-card-text>
            <b-card-text class="m-0 text-center">{{ $t('pages.to') }}</b-card-text>
            <b-card-text
              v-if="order.subtotal !== null"
              class="list-item-heading mb-1 truncate small text-center">{{ order.promotion.to_date }}
            </b-card-text>
            <b-card-text
              ...
              <b-icon-person/>
              <kbd>{{
                  $_.isEmpty(order.promotion.user_id) ? $t('pages.public') : (((order || {}).promotion || {}).user_id || {}).name || 'NaN'
                }}</kbd>
            </b-card-text>
          </b-card>
          <!--          promotion-->
          <b-card
            ...
            <template #header>
              <div class="d-flex justify-content-between">
                <h6 class="mb-0"><strong class="iconsminds-quill-3"/>{{ $t('pages.note') }}</h6>
              </div>
            </template>
          </b-card>
          <b-card
            ...
            <b-card-sub-title class="list-item-heading mb-1 d-flex justify-content-between">
            ...
            <b-table
                ...
                <div>
                  <router-link
                    ...
                    <thumbnail-image
                      ...
                      class-name="h-25"
                      :alt="(((((row||{}).item||{}).item||{}).locales||{})[$i18n.locale]||{}).name || ''"/>
                    <b-tooltip
                ...
                <div>
                  <b-link
                    ...
                    <div
                      ...
                      @click="isOffer=true">
                      <b-badge variant="outline-primary">- {{
                          row.item.discount_value !== null ? row.item.discount_value : 0
                        }}
                      </b-badge>
                      <b-icon-caret-down-fill v-if="row.item._showDetails && isOffer"/>
              ...
              <template #cell(order_details_customization)="row">
                <div>
                  ...
                  <span v-else class="d-flex justify-content-center list-item-heading">-</span>
                </div>
              </template>
              <template #cell(item.locales)="row">
                <div>
                  <router-link
                    ...
                    {{ (((((row || {}).item || {}).item || {}).locales || {})[$i18n.locale] || {}).name || '' }}
                    <b-tooltip
                ...
                <b-card>
                  <section v-if="isOffer">
                    <b-row align-h="between">
                      <b-card-title>
                        {{
                          (((((row || {}).item || {}).offer || {}).locales || {})[$i18n.locale] || {}).title || 'There is No Offer'
                        }}
                      </b-card-title>
                    ...
                    <div v-if="row.item.offer.item_id">
                      <b-card-sub-title>
                        {{
                          (((((row || {}).item || {}).offer || {}).locales || {})[$i18n.locale] || {}).description || undefined
                        }}
                      </b-card-sub-title>
                      <b-card-text class="d-flex justify-content-between">
                        {{ (((row || {}).item || {}).offer || {}).from_date || undefined }}<b>></b>
                        {{ (((row || {}).item || {}).offer || {}).to_date || undefined }}

                        <b-badge variant="outline-primary">
                          {{ (((row || {}).item || {}).offer || {}).discount_percentage * 100 || undefined }}%
                        </b-badge>
                      </b-card-text>
                    </div>
                  </section>
                  <section v-else>
                    ...
                    <div v-if="row.item.order_details_customization.length>0">
                      <b-card-text
                        ...
                        <b>{{
                            ((((e || {}).customization || {}).locales || {})[$i18n.locale] || {}).title || 'NaN'
                          }}</b>

                        <b-badge variant="outline-primary">
                          {{ $t('pages.aed') }} {{ (e || {}).added_value || 'NaN' }}
                        </b-badge>
                      </b-card-text>
                  ...
                  <template #footer v-if="!isOffer">
                    <b-row
                      ...
                      {{
                        ...
                        (((row || {}).item || {}).order_details_customization || []).map(x => ((x || {}).added_value || 0)).reduce((a, b) => Number(a) + Number(b), 0)
                      }}</em>
                    </b-row>
                  </template>
                </b-card>
...
export default {
      ...
      fields: [{
        ...
        },
        {
          ...
          formatter: (value, key, item) => {
            let list = value.map(x => ((((x || {}).customization || {}).locales || {})[this.$i18n.locale] || {}).title || 'NaN')
  ...
  methods: {
    ...
    formatDateTime(date) {
      ...
      return today.toLocaleDateString(this.$i18n.locale === 'en' ? "en-US" : 'ar-EG', options)
    },
      ...
      axios.get(`${apiUrl}orders/${id}`)
        ...
        .catch(error => {
          ...
          if (error.response.data.status == 404) {
            this.$notify("error", "This Order Not Found", null, null, {duration: 5000, permanent: false});
          } else {
            this.$notify("error", "NetWork Error", null, null, {duration: 5000, permanent: false});
          }
        })
    }
  },
