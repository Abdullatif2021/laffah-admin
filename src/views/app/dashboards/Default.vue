      ...
      <b-colxx xl="6" lg="12">
        ...
        <b-row>
          <b-colxx md="12" class="mb-4">
            <b-card :title="$t('dashboards.sales')">
              <div class="dashboard-line-chart" v-if="showChart">
        ...
        <b-row>
          ...
          >
            <recent-orders
              ...
              v-if="showData"
              :isLoad="isLoad"
              @refreshRecent="fetchRecentOrders()"
            ></recent-orders>
          </b-colxx>
        </b-row>
      </b-colxx>
...
export default {
  ...
  methods: {
    ...
    getRatingInfo() {
      return axios
        .get(
          "https://api-v2.laffahrestaurants.com/public/api/orders/rating-count"
        )
        .then((res) => {
          console.log({ res });
          this.doughnut_data.datasets[0]["data"].push(
            res.data.data.rating_one_tow
          );
          this.doughnut_data.datasets[0]["data"].push(
            res.data.data.rating_three_four
          );
          this.doughnut_data.datasets[0]["data"].push(
    ...
    getRecentOrders() {
      ...
      axios
        ...
        .then((response) => {
          ...
          this.diffArray = newDataIDS.filter(
            (x) => !currentDataIDS.includes(x)
          );
          // console.log(newDataIDS,this.diffArray,'IDs')
          if (this.diffArray.length > 0) {
            this.$notify("primary", "New Order Added", null, {
              duration: 5000,
              permanent: false,
            });
            localStorage.setItem(
              "recentorders",
              response.data.data.map((i) => i.id)
            );
          }
        ...
        .then((response) => {
          ...
          if (Array.isArray(result)) {
            let unique = result.reduce(
              (acc, x) =>
                acc.concat(
                  acc.find((y) => y.created_at === x.created_at) ? [] : [x]
                ),
              []
            );
            console.log(unique);
            self.lineChartData.datasets[0].data = unique.map((i) =>
              parseFloat(i.total)
            );
