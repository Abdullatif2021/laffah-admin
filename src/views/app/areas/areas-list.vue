  ...
  <b-row>
    <b-colxx lg="7" xl="7">
      <grid-body
        ...
        ref="grid"
        :addMeta="false"
        @editMetadata="setMetaRow($event)"
      ></grid-body>
    </b-colxx>
    <b-colxx lg="5" xl="5" class="mt-5">
      ...
      <div class="d-flex flex-column">
        <b-form v-if="isLoad" @submit.prevent="formSubmit">
          <b-form-group
          ...
          >
            ...
            />
            <b-form-invalid-feedback
              v-if="!$v.formActions.minimum_orders.required"
              >{{
          ...
          >
            ...
            />
            <b-form-invalid-feedback
              v-if="!$v.formActions.delivery_cost.required"
              >{{
          ...
          <div v-if="!disableForm" class="d-flex">
            <b-button
              v-if="!toggleState"
              type="submit"
              variant="primary"
            ...
            <b-button
              ...
              class="mt-4 mx-2 align-self-center align-self-baseline"
              >{{ $t("forms.cancel") }}
            </b-button>
          </div>
        </b-form>
...
export default {
  ...
  methods: {
    ...
    formSubmit() {
      if (!this.disableForm) {
        ...
        if (
          ...
          Object.keys(this.selectedBranch).length === 0 &&
          this.selectedBranch.constructor === Object
        ) {
          ...
          let data = {
            ...
            "area_minimum_order[minimum_orders]":
              this.formActions.minimum_orders,
          ...
          if (this.areaID != null) {
            ...
            axios //PUT method
              .post(this.apiBase + "/" + this.areaID, form_data)
              .then((res) => {
                this.toggleState = false;
                self.$nextTick(() => self.$refs.grid.getData());
                this.$notify("success", "Done", null, {
                  duration: 5000,
                  permanent: false,
                });
                this.resetForm();
    ...
    setMetaRow(e) {
      ...
      const objb = this.formActions.branches.filter(
        (i) => i.id == e.areaMinimumOrders[0].branch_id
      );
  ...
  created() {
    axios
      ...
      .then((res) => {
        ...
        this.formActions.cities = result.map((obj) => ({
          id: obj.id,
          name: obj.locales["en"].name,
        }));
    ...
    axios
      ...
      .then((res) => {
        let result = res.data.data;
        this.formActions.branches = result.map((obj) => ({
          id: obj.id,
          name: obj.locales["en"].name,
        }));
        // let result = data.map(({ id, locales['en'].name }) => ({id, locales['en'].name}));
