...
const getters = {
    ...
    getNotifications: (state) => {
        return Object.values(state.notificationsList)
            .map(o => o)
    },
    getSortColumn: (state) => {
...
const mutations = {
    ...
    SET_NotificationsList(state, payload) {
        localStorage.setItem('notesData', payload.map(i => i.id));
...
const actions = {
    loadNotificationsList({ commit, dispatch, state }) {
        return axios
            .get(apiUrl + modelUrl + '?order_by=created_at&order_dir=DESC&limit=20&offset=0')
            ...
            .then(({ data, total_unread }) => {
                if (Array.isArray(data) && data.length > 0) {
                    ...
                    if (diffArray.length > 0) {
                        console.log(diffArray.length, 'diffArray')
                        Vue.prototype.$notify("primary", "New notification Added", null, { duration: 5000, permanent: false });
                    }
                }
