  ...
  >
    <b-card-body class="d-flex justify-content-between align-content-start">
      ...
      <div class="pl-3 flex-grow-1">
        <p class="mb-0 text-uppercase font-weight-bolder">
          {{ order.contact_name }}
        </p>
    ...
    <b-card-text
      id="due-date"
      class="text-danger text-center font-weight-bolder"
    ...
    <div
      ...
      class="d-flex text-danger text-center font-weight-bolder justify-content-center gap-4"
    >
      <!-- <i class="iconsminds-motorcycle"></i> -->
      <p>Delivery</p>
    </div>
    ...
    <div class="d-flex justify-content-between align-content-center my-2">
      <b-button
        ...
        size="sm"
        pill
        @click="showOrderDetails(order, $event)"
      >
        <span v-if="fetchingOrder !== order.id">{{ $t("pages.view") }}</span>
        <div v-if="fetchingOrder === order.id" class="spinner d-inline-block">
