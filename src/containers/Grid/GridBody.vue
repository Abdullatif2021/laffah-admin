    ...
    <vuetable
     ...
     <template slot="image" slot-scope="props">
      <ThumbnailImage
       ...
       rounded
       small
       :src="order_index !== undefined ? props.rowData.item.image : props.rowData.image"
       class-name="" />
      <ThumbnailImage
     ...
     <template slot="price" slot-scope="props">
      <b-badge
       ...
       <span>{{
                  `${props.rowData.price || (props.rowData.min_orders || {}).delivery_cost || 0} ${$t('pages.aed')}`
                }}</span>
      </b-badge>
     </template>
     <template slot="price2" slot-scope="props">
      <b-badge
       pill
       variant="outline-primary">
       <span>{{ `${props.rowData.min_orders.minimum_orders} ${$t('pages.aed')}` }}</span>
      </b-badge>
     </template>
     <template slot="visible" slot-scope="props">
      <b-badge
       pill
       :variant="props.rowData.visible=='1' ? 'outline-success' : 'outline-danger'">
       <span v-if="props.rowData.visible=='1'">{{ $t("active") }}</span>
       <span v-else-if="props.rowData.visible=='0'">{{ $t("not-active") }}</span>
      </b-badge>
     </template>
     ...
     <template slot="active" slot-scope="props">
      <b-badge
       pill
       :variant="props.rowData.active=='1' || props.rowData.open_delivery=='1'  ? 'outline-success' : 'outline-danger'">
       <span v-if="props.rowData.active=='1' || props.rowData.open_delivery=='1'">{{ $t("active") }}</span>
       <span v-else-if="props.rowData.active=='0' || props.rowData.open_delivery=='0'">{{
                  $t("not-active")
                }}</span>
      </b-badge>
     </template>
     <template slot="paymentWay" slot-scope="props">
      <h6 v-if="props.rowData.payment_method=='1'">{{ $t("cash-on-delivery") }}</h6>
      <h6 v-else-if="props.rowData.payment_method=='2'">{{ $t("payment-online") }}</h6>
     </template>
     ...
     <template slot="applicants" slot-scope="props">
      <a class="custom-control-icon" href="#" @click.prevent="showApplicants(props.rowData.id)">
       <b-button
        size="xs"
        class="mb-1 btn-shadow"
        variant="primary">{{ $t('menu.show-applicants') }}
       </b-button>
      </a>
     </template>
     <template slot="cv" slot-scope="props">
      <a class="custom-control-icon" target="_blank" :href="props.rowData.cv">
       <b-button
        size="xs"
        class="mb-1 btn-shadow"
        variant="primary">{{ $t('menu.download-cv') }}
       </b-button>
      </a>
     </template>
     <template slot="orderDetails" slot-scope="props">
      <a class="custom-control-icon" href="#" @click.prevent="orderDetails(props.rowData.id)">
       <b-button
        size="xs"
        class="mb-1 btn-shadow"
        variant="primary">{{ $t('menu.order-details') }}
       </b-button>
      </a>
     </template>
     ...
     <template slot="actions" slot-scope="props">
      <b-dropdown
       ...
       <template #button-content>
        <div class="py-0">
         <b-link
          ...
          <span for="edit" class="p-2 font-weight-bold">{{ $t('forms.edit') }}</span>
         </b-link>
         <b-link
          id="edit"
          class="d-flex align-items-center text-white px-2"
          ...
          @click="itemActions('edit',props.rowData)"><i style='font-size:20px' class='iconsminds-gear-2 d-flex'></i>
          <span for="edit" class="p-2 font-weight-bold">{{ $t('forms.edit') }}</span>
         </b-link>
         <b-link
          id="edit"
          class="d-flex align-items-center  text-white px-2"
          v-else-if="(apiBase.includes('metadata') || apiBase.includes('areas')) && addEdit"
         ...
         <b-link
          ...
          @click="onActionClicked('edit-meta',props.rowData)">
          <i style='font-size:20px' class='iconsminds-gear-2 d-flex'></i>
          <span for="edit" class="p-2 font-weight-bold">{{ $t('forms.display') }}</span>
         </b-link>
         <b-link
          v-else
          id="edit"
       ...
       <b-dropdown-item
        href="#"
        v-if="!apiBase.includes('metadata') && addMeta &&!apiBase.includes('customization')"
        @click.prevent=" metaData({data:props.rowData})">
        <i class="simple-icon-puzzle  color-theme-1"></i><span class="mx-1">{{ $t('forms.meta') }}</span>
       </b-dropdown-item>
       <b-dropdown-item
        v-if="addAttach"
        class=""
        ...
        <i class="simple-icon-paper-clip  color-theme-1"></i><span class="mx-1">{{
                  $t('forms.attachments')
                }}</span>
       </b-dropdown-item>
       <b-dropdown-item
       ...
       <b-dropdown-item
        v-if="(apiBase.includes('metadata') || apiBase.includes('areas')) && addEdit"
        @click="onActionClicked('edit-meta',props.rowData)">
        <i class="iconsminds-quill-3
  color-theme-1"></i><span class="mx-1">{{ $t('forms.edit') }}</span>
       </b-dropdown-item>
 ...
 methods: {
  ...
  dataManager2(sortOrder, pagination) {
   ...
   if (this.search) {
    ...
    let dataFields = this.fieldsNames.map(function (el) {
      return el.name;
     })
     .filter(function (e) {
  ...
  onActionClicked(action, data) {
   ...
   if (action === 'edit-item') {
    if (this.attachType !== undefined) {
     this.$router.push(`../${this.addRow}/${this.attachType}/${this.id}/${data.data.id}`)
    } else {
  ...
  deleteItem(id) {
   ...
   if (this.id !== undefined && !this.apiBase.includes('customizations')) {
    axios.delete(`/${this.apiBase}/${this.id}/${id}`)
     .then(resp => {
      this.getData()
     ...
     .catch(error => {
      this.getData()
      this.$notify("error", "Deleted Failed", null, { duration: 5000, permanent: false });
      console.log(error);
     })
   } else {
    ...
    axios.delete(`/${tempApiBase}/${id}`)
     ...
     .catch(error => {
      ...
      this.$notify("error", "Deleted Failed", "Try again later", { duration: 5000, permanent: false });
      console.log(error);
     })
   }
   ...
   if (this.apiBase == "orders") {
    ...
    if (this.type !== undefined) {
     ...
     if (this.id !== undefined) {
      axios
       .get(this.apiBase.includes('customizations?customization_group_id=') ? `${this.apiBase}&orderBy[]=created_at&orderBy[]=desc` : `${this.apiBase}/${this.id}?&orderBy[]=created_at&orderBy[]=desc`)
       .then(response => {
